...
`grantTypes` | [Array&lt;GrantTypeEnum&gt;](GrantTypeEnum.md)
`clientId` | string
`clientSecret` | string
...
```typescript
import type { PatchedOAuth2ProviderRequest } from '@goauthentik/api'
...
} satisfies PatchedOAuth2ProviderRequest
...
const exampleParsed = JSON.parse(exampleJSON) as PatchedOAuth2ProviderRequest
