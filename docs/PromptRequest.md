...
`initialValue` | string
`order` | number
`promptstageSet` | [Array&lt;StageRequest&gt;](StageRequest.md)
`subText` | string
...
```typescript
import type { PromptRequest } from '@goauthentik/api'
...
const exampleParsed = JSON.parse(exampleJSON) as PromptRequest
