...
import type { CoreApplicationsSetIconCreateRequest } from '@goauthentik/api';
...
async function example() {
  ...
  const body = {
    ...
    // boolean (optional)
    clear: true,
  } satisfies CoreApplicationsSetIconCreateRequest;

  try {
    const data = await api.coreApplicationsSetIconCreate(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
import type { CoreApplicationsSetIconUrlCreateRequest } from '@goauthentik/api';
...
async function example() {
  ...
  const body = {
    ...
    // FilePathRequest
    filePathRequest: ...,
  } satisfies CoreApplicationsSetIconUrlCreateRequest;

  try {
    const data = await api.coreApplicationsSetIconUrlCreate(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
> PaginatedBrandList coreBrandsList(brandUuid, brandingDefaultFlowBackground, brandingFavicon, brandingLogo, brandingTitle, clientCertificates, _default, domain, flowAuthentication, flowDeviceCode, flowInvalidation, flowRecovery, flowUnenrollment, flowUserSettings, ordering, page, pageSize, search, webCertificate)
...
> PaginatedGroupList coreGroupsList(attributes, includeChildren, includeUsers, isSuperuser, membersByPk, membersByUsername, name, ordering, page, pageSize, search)
...
> Group coreGroupsRetrieve(groupUuid, includeChildren, includeUsers)
...
async function example() {
  ...
  const body = {
    ...
    // string (optional)
    identifier: identifier_example,
    // 'api' | 'app_password' | 'recovery' | 'verification' (optional)
...
| **intent** | `api`, `app_password`, `recovery`, `verification` |  | [Optional] [Defaults to `undefined`] [Enum: api, app_password, recovery, verification] |
...
> PaginatedUserList coreUsersList(attributes, dateJoined, dateJoinedGt, dateJoinedLt, email, groupsByName, groupsByPk, includeGroups, isActive, isSuperuser, lastUpdated, lastUpdatedGt, lastUpdatedLt, name, ordering, page, pageSize, path, pathStartswith, search, type, username, uuid)
...
async function example() {
  ...
  const body = {
    ...
    // string | A search term. (optional)
    search: search_example,
    // Array<'external' | 'internal' | 'internal_service_account' | 'service_account'> (optional)
    type: ...,
    // string (optional)
...
| **search** | `string` | A search term. | [Optional] [Defaults to `undefined`] |
| **type** | `external`, `internal`, `internal_service_account`, `service_account` |  | [Optional] [Enum: external, internal, internal_service_account, service_account] |
...
| Name | Type | Description  | Notes |
|------------- | ------------- | ------------- | -------------|
| **emailStage** | `string` |  | [Defaults to `undefined`] |
| **id** | `number` | A unique integer value identifying this User. | [Defaults to `undefined`] |
