...
export function instanceOfOAuth2ProviderSetupURLs(value: object): value is OAuth2ProviderSetupURLs {
    ...
    if (!('dcrRegistration' in value) || value['dcrRegistration'] === undefined) return false;
    return true;
}
...
export function OAuth2ProviderSetupURLsToJSONTyped(value?: Omit<OAuth2ProviderSetupURLs, 'issuer'|'authorize'|'token'|'user_info'|'provider_info'|'logout'|'jwks'|'dcr_registration'> | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
