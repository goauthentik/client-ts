...
> Array&lt;App&gt; adminModelsList(filterHasAttributes)
...
async function example() {
  ...
  } satisfies AdminModelsListRequest;

  try {
    const data = await api.adminModelsList(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| Name | Type | Description  | Notes |
|------------- | ------------- | ------------- | -------------|
| **filterHasAttributes** | `boolean` |  | [Optional] [Defaults to `undefined`] |
